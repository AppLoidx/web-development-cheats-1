...
<details>
    <summary><strong>Пример абсолютного и относительного пути</strong></summary>
Если у вас возникли трудности с понятие абсолютного и относительного пути, то можно привести такой пример:

Допустим, запрос поступил по пути `/itmo/ivt.html` и если нужно перейти на соседнюю страницу `pikt.html`, тогда следующие операции выполняют диспетчеризацию по одному и тому же пути:
* `ServletRequest.getRequestDispatcher("pikt.html")`
* `ServletContext.getRequestDispatcher("/itmo/pikt.html")`
</details>
